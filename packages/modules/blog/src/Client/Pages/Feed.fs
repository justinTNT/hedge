...
let update msg model =
    ...
    | LoadFeed ->
        ...
        Cmd.OfPromise.either Client.ClientGen.blogGetFeed { Cursor = None } GotFeed (fun ex -> GotFeed (Error ex.Message))
    ...
    | LoadMoreFeed ->
        ...
        | Some feed when feed.NextCursor.IsSome && not model.FeedLoadingMore ->
            ...
            Cmd.OfPromise.either Client.ClientGen.blogGetFeed { Cursor = feed.NextCursor } GotMoreFeed (fun ex -> GotMoreFeed (Error ex.Message))
        | _ -> model, Cmd.none
