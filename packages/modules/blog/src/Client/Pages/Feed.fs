...
let update msg model =
    ...
    | LoadFeed ->
        ...
        Cmd.OfPromise.either Client.ClientGen.blogGetFeed "start" GotFeed (fun ex -> GotFeed (Error ex.Message))
    ...
    | LoadMoreFeed ->
        ...
        | Some feed when feed.NextCursor.IsSome && not model.FeedLoadingMore ->
            ...
            Cmd.OfPromise.either Client.ClientGen.blogGetFeed feed.NextCursor.Value GotMoreFeed (fun ex -> GotMoreFeed (Error ex.Message))
        | _ -> model, Cmd.none
