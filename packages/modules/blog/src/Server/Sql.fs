...
let itemBySlug =
    sprintf "SELECT id, title, link, image, extract, owner_comment, slug, created_at, updated_at, view_count, deleted_at FROM %s WHERE slug = ?" Tables.microblogItem
...
let feedFirstPage =
    sprintf "SELECT id, title, link, image, extract, owner_comment, article_date, slug, created_at, updated_at, view_count, deleted_at FROM %s WHERE deleted_at IS NULL ORDER BY article_date DESC, id DESC LIMIT ?" Tables.microblogItem

// Bind: [cursorTs, cursorTs, cursorId, limit].
let feedAfterCursor =
    sprintf "SELECT id, title, link, image, extract, owner_comment, article_date, slug, created_at, updated_at, view_count, deleted_at FROM %s WHERE deleted_at IS NULL AND (article_date < ? OR (article_date = ? AND id < ?)) ORDER BY article_date DESC, id DESC LIMIT ?" Tables.microblogItem
...
let itemsByTag =
    sprintf "SELECT i.* FROM %s i JOIN %s it ON i.id = it.item_id JOIN %s t ON it.tag_id = t.id WHERE t.name = ? AND i.deleted_at IS NULL ORDER BY i.article_date DESC, i.id DESC LIMIT ?" Tables.microblogItem Tables.itemTag Tables.tag

// Bind: [tag, cursorTs, cursorTs, cursorId, limit].
let itemsByTagAfter =
    sprintf "SELECT i.* FROM %s i JOIN %s it ON i.id = it.item_id JOIN %s t ON it.tag_id = t.id WHERE t.name = ? AND i.deleted_at IS NULL AND (i.article_date < ? OR (i.article_date = ? AND i.id < ?)) ORDER BY i.article_date DESC, i.id DESC LIMIT ?" Tables.microblogItem Tables.itemTag Tables.tag
