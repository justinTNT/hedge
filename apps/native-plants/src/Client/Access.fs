...
[<Import("storedAdminKey", "./access-client.mjs")>]
let storedKey () : string = jsNative
[<Import("readCapabilities", "./access-client.mjs")>]
let private read (key:string) : JS.Promise<Capabilities> = jsNative
...
let update msg model =
    ...
    | Refresh ->
        ...
        if model.Loading && key=model.Key then model,Cmd.none
            ...
            next,Cmd.OfPromise.either read key (fun result->Loaded(next.Epoch,Ok result)) (fun ex->Loaded(next.Epoch,Error ex.Message))
...
[<Emit("window.addEventListener('storage',e=>{if(e.key===null || e.key==='adminKey')$0()})")>]
let listen (refresh:unit->unit) : unit = jsNative
