...
let private contentRevision (details: PlantDetail array) : JS.Promise<string> = jsNative
...
let private build (rows: obj array) (photoRows: obj array) =
    ...
    let details = rows |> Array.map (fun row ->
        ...
        { Card=card;Photos=gallery;Sections=labels |> List.choose (fun (key,label) ->
    ...
    { ById=details |> List.map (fun d -> d.Card.Id,d) |> Map.ofList
      Public={ Plants=cards;Families=families cards;Forms=facets (fun p -> p.Forms) cards
               ...
               Revision=string (now ());RefreshSeconds=15 } }
        ...
        | None ->
            ...
            let pending = promise {
                try
                    ...
                    |]
                    let catalogue = build result.[0].results result.[1].results
                    let! revision = catalogue.ById |> Map.toArray |> Array.map snd |> contentRevision
