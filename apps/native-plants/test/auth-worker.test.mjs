...
test('capability discovery uses the same owner and curator policy as review and admin',async()=>{
  ...
  const none={CanEditCatalogue:false,CanReview:false,CanIdentify:false};
  ...
  assert.deepEqual(await access({'X-Admin-Key':f.env.ADMIN_KEY}),{CanEditCatalogue:true,CanReview:true,CanIdentify:true});
  ...
  assert.deepEqual(await access({Cookie:cookie}),{CanEditCatalogue:false,CanReview:true,CanIdentify:false});
