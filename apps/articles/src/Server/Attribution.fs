...
/// the module that owns the table and composed per-site in `Server.AttributionPolicy`
/// (justat = articles + blog; ndct = articles only). Every statement shares the bind
/// shape [toId; toId; fromId] (see each module's `reassignComments`), so one batch with
/// one bind array reassigns them all atomically.
...
// sum across all of `AttributionPolicy.commentTables` rather than just articles_comments.
// The table names come from that per-site list (not user input), so interpolating them is
// safe; identity ids stay parameterised.
