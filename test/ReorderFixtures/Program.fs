...
let main _ =
    ...
    let mOk, _ = BFeed.update (BT.GotFeed (5, Ok (bFeedResp [ "b1"; "b2" ] None))) m
    check "blog.feed.currentGen.accepts" (mOk.Feed.IsSome && mOk.Feed.Value.Items.Length = 2)
    let mStale, cStale = BFeed.update (BT.GotFeed (4, Ok (bFeedResp [ "x" ] None))) m
    ...
    let mFailStale, _ = BFeed.update (BT.GotFeed (4, Error "boom")) mf
    check "blog.feed.staleFailure.ignored" (mFailStale.Error.IsNone)
    let mFailCur, _ = BFeed.update (BT.GotFeed (5, Error "boom")) mf
    ...
    let mi = { bModel with Route = [ "b1" ]; LoadGen = 7; Error = Some "old" }
    let miOk, ciOk = BItem.update (BT.GotItem (7, Ok (bGetItemResp "b1"))) mi
    ...
    let miStale, ciStale = BItem.update (BT.GotItem (6, Ok (bGetItemResp "b1"))) mi
    ...
    let mMerge, _ = BFeed.update (BT.GotMoreFeed (9, Some "c1", Ok (bFeedResp [ "b2" ] None))) mp
    check "blog.more.matchingCursor.merges" (mMerge.Feed.Value.Items.Length = 2 && not mMerge.FeedLoadingMore)
    let mWrongCur, _ = BFeed.update (BT.GotMoreFeed (9, Some "cX", Ok (bFeedResp [ "zz" ] None))) mp
    check "blog.more.wrongCursor.noMerge" (mWrongCur.Feed.Value.Items.Length = 1 && not mWrongCur.FeedLoadingMore)
    let mMoreStale, _ = BFeed.update (BT.GotMoreFeed (8, Some "c1", Ok (bFeedResp [ "zz" ] None))) mp
    ...
    let mLate, cLate = BItem.update (BT.GotSubmitComment (2, Ok (bSubmitCommentResp "cm1" "b1"))) mc
    check "blog.comment.staleRev.keepsDraft" (mLate.CommentDraft = "newer text" && isDrop cLate)
    let mClear, cClear = BItem.update (BT.GotSubmitComment (3, Ok (bSubmitCommentResp "cm1" "b1"))) mc
    ...
    let mNew, _ = BNew.update (BT.GotSubmitItem (Ok (bSubmitItemResp "b9"))) mn
    ...
    let mAfterLeave, cAfterLeave = BItem.update (BT.GotItem (4, Ok (bGetItemResp "b1"))) mLeft
    ...
    let mTagStale, cTagStale = BTag.update (BT.GotMoreTagItems (2, Some "c1", Ok { Tag = "fp"; Items = [ bFeedItem "zz" ]; NextCursor = None })) mt
    check "blog.tag.more.staleGen.drops" (mTagStale.TagItems.Value.Items.Length = 1 && mTagStale.TagLoadingMore && isDrop cTagStale)
    let mTagMerge, _ = BTag.update (BT.GotMoreTagItems (3, Some "c1", Ok { Tag = "fp"; Items = [ bFeedItem "b2" ]; NextCursor = None })) mt
    ...
    let amOk, _ = AFeed.update (AT.GotFeed (5, Ok (aFeedResp [ "a1"; "a2" ] None))) am
    check "art.feed.currentGen.accepts" (amOk.Feed.IsSome && amOk.Feed.Value.Items.Length = 2)
    let amStale, amsC = AFeed.update (AT.GotFeed (4, Ok (aFeedResp [ "x" ] None))) am
    ...
    let ai = { aModel with Route = [ "a1" ]; LoadGen = 7; Error = Some "old" }
    let aiOk, aiC = AItem.update ctx (AT.GotItem (7, Ok (aPostResp "a1"))) ai
    check "art.item.currentGen.accepts" (aiOk.CurrentItem.IsSome && aiOk.Error.IsNone && notDrop aiC) // batch: SetDocTitle+connect
    let aiStale, aiSC = AItem.update ctx (AT.GotItem (6, Ok (aPostResp "a1"))) ai
    ...
    let aLate, aLateC = AItem.update ctx (AT.GotSubmitComment (2, Ok (aSubmitCommentResp "cm1" "a1"))) ac
    ...
    let aClear, aClearC = AItem.update ctx (AT.GotSubmitComment (3, Ok (aSubmitCommentResp "cm1" "a1"))) ac
